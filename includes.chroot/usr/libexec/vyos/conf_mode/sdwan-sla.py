...
def update_sla_config(sla_config, sla_profile, sla_data):
    ...
    updated = False
    for entry, entry_data in config_data.items():
        if entry_data.get("sla_profile") == sla_profile or entry_data.get("sla-profile") == sla_profile:
            if entry_data.get("sla") != sla_data:
                if "sla" in entry_data:
                    entry_data["sla"] = sla_data
                else:
                    entry_data["sla_profile"] = sla_data
                updated = True
    if updated:
        ...
        for s in slas:
            if os.path.exists(sla_config):
                ...
                for e in existing_data:
                    if existing_data[e]['sla_profile'] == s:
                        raise ConfigError(f"Error, SLA Profile is in use on {e}")
            else:
                return None
    else:
        for s in slas:
            try:
                config = s['sla-profiles'][s]
                update_sla_config(sla_config, s, config)
            except:
                pass
    return None
